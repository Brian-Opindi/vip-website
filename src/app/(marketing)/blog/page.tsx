...
import { NavMenu } from "@/app/(marketing)/navigation-menu"; // Add this import
...
const BlogPage = () => (
	<>
		<div className="absolute left-0 right-0 z-10 mx-auto max-w-6xl flex justify-center bg-black rounded-md">
			<NavMenu />
		</div>
		<section className="min-h-screen bg-white py-16 px-4">
			<div className="max-w-4xl mx-auto">
				<h1 className="text-4xl font-bold mb-8 font-urbanist text-coffee mt-10">VIP Blog</h1>
				<div className="grid gap-8 md:grid-cols-2">
					{posts.map((post) => (
						<article
							key={post.slug}
							className="rounded-2xl shadow-lg overflow-hidden bg-gray-50 hover:shadow-2xl transition"
						>
							<img
								src={post.image}
								alt={post.title}
